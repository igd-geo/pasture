...
fn generate_line_model<T: BorrowedBuffer>(
    buffer: &T,
...
fn generate_plane_model<T: BorrowedBuffer>(
    buffer: &T,
    distance_threshold: f64,
) -> (Plane, Vec<usize>) {
...
pub fn ransac_plane_par<T: BorrowedBuffer + Sync>(
    buffer: &T,
    distance_threshold: f64,
...
///
/// If the size of the buffer is < 3.
pub fn ransac_plane_serial<T: BorrowedBuffer + Sync>(
    buffer: &T,
    distance_threshold: f64,
    num_of_iterations: usize,
) -> (Plane, Vec<usize>) {
...
pub fn ransac_line_par<T: BorrowedBuffer + Sync>(
    buffer: &T,
    distance_threshold: f64,
    num_of_iterations: usize,
) -> (Line, Vec<usize>) {
...
///
/// If the size of the buffer is < 2.
pub fn ransac_line_serial<T: BorrowedBuffer>(
    buffer: &T,
    distance_threshold: f64,
    num_of_iterations: usize,
) -> (Line, Vec<usize>) {
