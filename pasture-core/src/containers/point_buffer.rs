...
use super::{
    ...
    BufferViewRef, DefaultFromPointLayout, IndexBuffer, IndexBufferMut, PolymorphicStorage,
    PolymorphicStorageMut,
};
...
impl<T: BufferStorage> PointBuffer<T> {
    ...
    pub fn into_polymorphic_buffer(self) -> PointBuffer<PolymorphicStorage>
    where
        T: 'static,
    {
        PointBuffer::new(
            PolymorphicStorage::from_storage(self.storage),
            self.point_layout,
        )
    }
}
...
impl<T: BufferStorageMut> PointBuffer<T> {
    ...
    pub fn into_polymorphic_buffer_mut(self) -> PointBuffer<PolymorphicStorageMut>
    where
        T: 'static,
    {
        PointBuffer::new(
            PolymorphicStorageMut::from_storage(self.storage),
            self.point_layout,
        )
    }
}
